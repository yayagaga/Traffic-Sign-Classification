...
clf_svm = svm.SVC(C = 70, decision_function_shape = 'ovr', gamma = 'scale', kernel = 'rbf')
clf_svm.fit(X_train, Y_train)
print("Training set score: %0.6f" % clf_svm.score(X_train, Y_train))
...
scores = cross_val_score(clf_svm, x, y, cv=3)
print("Mean accuracy of cross validation: %0.6f (+/- %0.4f)" % (scores.mean(), scores.std() * 2))

# Classification report
cm = confusion_matrix(Y_test, Y_test_pred, labels=clf_svm.classes_)
...
plt.show()
print(classification_report(Y_test, Y_test_pred, labels=clf_svm.classes_))
p_class, r_class, f_class, support_micro = precision_recall_fscore_support(
                                 Y_test, Y_test_pred, labels=clf_svm.classes_)
...
# save the model
pickle.dump(clf_svm,open('clf_svm.sav','wb'))
